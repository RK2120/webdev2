...
const renderHTMLQueue = async (workerPool, activity, htmlComponentRendererPath, pages, stage = _types.Stage.BuildHTML) => {
  ...
  const renderHTML = stage === `build-html` ? workerPool.renderHTMLProd : workerPool.renderHTMLDev;
  const uniqueUnsafeBuiltinUsedStacks = new Set();

  try {
    await _bluebird.default.map(segments, async pageSegment => {
      const htmlRenderMeta = await renderHTML({
        envVars,
    ...
    if (e === null || e === void 0 ? void 0 : (_e$context = e.context) === null || _e$context === void 0 ? void 0 : _e$context.unsafeBuiltinsUsageByPagePath) {
