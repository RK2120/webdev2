...
const validateContentfulAccess = async pluginOptions => {
  ...
  await fetch(`https://${pluginOptions.host}/spaces/${pluginOptions.spaceId}/environments/${pluginOptions.environment}/content_types`, {
    ...
    if (!ok) {
      const errorMessage = `Cannot access Contentful space "${maskText(pluginOptions.spaceId)}" on environment "${pluginOptions.environment} with access token "${maskText(pluginOptions.accessToken)}". Make sure to double check them!`;
