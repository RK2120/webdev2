...
module.exports = async function cacheImage(store, image, options) {
  ...
  const absolutePath = resolve(CACHE_DIR, `${name}-${optionsHash}${ext}`); // Query the filesystem for file existence

  const alreadyExists = await pathExists(absolutePath); // Whether the file exists or not, if we are downloading it then await
  ...
  if (inFlight) {
    ...
    await downloadPromise; // When the file is downloaded, remove the promise from the cache
